...
// Created: 2024/04/29
// Description:
// This file contains the implementation of the BoucWenLRB class.
...
void* OPS_BoucWenLRB()
{
    ...
    double gamma2 = -0.173;                 // shape parameter of Z2
    double c1 = 0.018;                     // parameter controls degradation
    int flagLH = 0;                         // boolean flag to control whether lead core heating is enabled (1) or disabled (0)
    ...
    int flagIH = 0;                         // boolean flag to control whether initial hardening is enabled (1) or disabled (0)
    double c2 = 0.6;                        // parameter controls initial hardening
    double c3 = 1.2;                        // parameter controls initial hardening
    double cd = 0.0;                        // viscous damping parameter
    ...
    while (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        if (strcmp(type, "-alpha1") == 0) {
            ...
            if (OPS_GetDoubleInput(&numData, &cd) != 0) {
                opserr << "WARNING: invalid cd for element" << eleTag << endln;
                return 0;
            }
        }
...
int BoucWenLRB::update()
{
    ...
    if (uh > DMplusC) {
        DMplus = uh;
        DMminus = DMminusC + (DMplus - DMplusC);
        DM = DMC;
    }
    if (uh < DMminusC) {
        DMplus = DMplusC;
        DMminus = uh;
        DM = DMC - (DMminus - DMminusC);
    }
    // get KL
    KL = exp(-c1 * pow(DM / Tr, 3));
    ...
    if (sqrt(pow(delta_ub(1), 2) + pow(delta_ub(2), 2)) > DBL_EPSILON) {
        ...
        double amp2 = KL * alphaL * kInit * uy;
        double dt = (this->getDomain())->getDT();
        // set shear force
        qb(1) = ke * ub(1) + KI * KT1 * qYield * z1(0) + amp2 * z2(0) + cd * ubdot(1);
        qb(2) = ke * ub(2) + KI * KT1 * qYield * z1(1) + amp2 * z2(1) + cd * ubdot(2);
        // set tangent stiffness
        Kb(1, 1) = ke + KI * KT1 * qYield * dz1du(0, 0) + amp2 * dz2du(0, 0) + cd / dt;
        Kb(1, 2) = KI * KT1 * qYield * dz1du(0, 1) + amp2 * dz2du(0, 1);
        Kb(2, 1) = KI * KT1 * qYield * dz1du(1, 0) + amp2 * dz2du(1, 0);
        Kb(2, 2) = ke + KI * KT1 * qYield * dz1du(1, 1) + amp2 * dz2du(1, 1) + cd / dt;
    }
...
void BoucWenLRB::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        ...
        s << "  DMplus: " << DMplus << "  DMminus: " << DMminus
            << "  DM: " << DM << endln;
    ...
    OPS_Stream& output)
{
    ...
    {
        output.tag("ResponseType", "DMplus");
        output.tag("ResponseType", "DMminus");
...
int BoucWenLRB::getResponse(int responseID, Information& eleInfo)
{
    ...
    case 9:  // parameters that vary with time
        params(0) = DMplus / Tr; params(1) = DMminus / Tr;
        params(2) = DM / Tr; params(3) = dTL;
