...
Matrix BoucWenHDRB::theMatrix(12, 12);
Vector BoucWenHDRB::theVector(12);


static int tag = 0;  // Tag to identify if bearing has failed in buckling
void* OPS_BoucWenHDRB()
{
    int ndm = OPS_GetNDM();
    if (ndm != 3) {
        opserr << "BoucWenHDRB is only available for ndm=3 three-dimensional models\n";
        return 0;
    }
    ...
    if (ndf != 6) {
        ...
        opserr << ", for space problem need 6 - BoucWenHDRB\n";
        return 0;
    }
    ...
    if (numArgs == 0) { // parallel processing
        theEle = new BoucWenHDRB();
        return theEle;
    }

    if (numArgs < 15) {
        opserr << "WARNING insufficient arguments\n";
        opserr << "Want: BoucWenHDRB eleTag Nd1 Nd2 kInit fy alphaL Tr -P matTag -T matTag -My matTag -Mz matTag <-alpha1 alpha1> <-beta1 beta1> <-gamma1 gamma1> <-eta eta> <-alpha2 alpha2> <-beta2 beta2> <-gamma2 gamma2> <-c1 c1> <-c2 c2> <-orient x1 x2 x3 y1 y2 y3> <-sDratio sDratio> <-mass mass>\n";
        return 0;
    }
    ...
    double beta2 = 0.03;                    // shape parameter of Z2
    double gamma2 = -0.05;                  // shape parameter of Z2
    double cd = 2.4E-2;                     // parameter controls degradation
    double cm = 9E2;                      // parameter controls m for Z2
    ...
    while (OPS_GetNumRemainingInputArgs() > 0) {
        ...
        if (strcmp(type, "-alpha1") == 0) {
            ...
            numData = 1;
            if (OPS_GetDoubleInput(&numData, &cd) != 0) {
            ...
            numData = 1;
            if (OPS_GetDoubleInput(&numData, &cm) != 0) {
            ...
            opserr << "WARNING: unknown type: " << type << endln;
            return 0;
        }
    }

    theEle = new BoucWenHDRB(iData[0], iData[1], iData[2],
    ...
    KbInit(6, 6), theLoad(12)
{
    ...
    if (connectedExternalNodes.Size() != 2) {
        opserr << "BoucWenHDRB::BoucWenHDRB() - element: "
    ...
    if (materials == 0) {
        opserr << "BoucWenHDRB::BoucWenHDRB() - "
...
BoucWenHDRB::BoucWenHDRB()
    : Element(0, ELE_TAG_BoucWenHDRB), connectedExternalNodes(2),
    kInit(0), fy(0), alphaL(0), Tr(0),
    alpha1(1.0), beta1(0.5),
    gamma1(0.5), eta(1.0),
    alpha2(0.01), beta2(0.03),
    gamma2(-0.05), cd(2.4E-2), cm(9E2),
    x(0), y(0),
    sDratio(0.5), mass(0),
...
int BoucWenHDRB::getNumExternalNodes() const
{
    return 2;
}


const ID& BoucWenHDRB::getExternalNodes()
{
    return connectedExternalNodes;
}


Node** BoucWenHDRB::getNodePtrs()
{
    return theNodes;
}


int BoucWenHDRB::getNumDOF()
{
    return 12;
}


void BoucWenHDRB::setDomain(Domain* theDomain)
{
    ...
    if (!theNodes[0] || !theNodes[1]) {
        if (!theNodes[0]) {
            opserr << "WARNING BoucWenHDRB::setDomain() - Nd1: "
                << connectedExternalNodes(0)
                << " does not exist in the model for";
        }
        else {
            opserr << "WARNING BoucWenHDRB::setDomain() - Nd2: "
    ...
    if (dofNd1 != 6) {
        opserr << "BoucWenHDRB::setDomain() - node 1: "
            << connectedExternalNodes(0)
            << " has incorrect number of DOF (not 6).\n";
        return;
    }
    if (dofNd2 != 6) {
        opserr << "BoucWenHDRB::setDomain() - node 2: "
            << connectedExternalNodes(1)
            << " has incorrect number of DOF (not 6).\n";
        return;
    }
...
int BoucWenHDRB::commitState()
{
    ...
    // commit trial history variables for temporary hardening
    ubMaxC = ubMax;
...
int BoucWenHDRB::revertToLastCommit()
{
    int errCode = 0;
    ...
    // revert trial history variables for temporary hardening
    ubMax = ubMaxC;
...
int BoucWenHDRB::update()
{
    ...
    if (KH_flag == 0) {
        ...
        KH = 0.671 * ubMaxC / Tr + 0.831;
    }

    // get amp2
    double uhC = sqrt(pow(ubC(1), 2) + pow(ubC(2), 2));
    amp2 = cm * pow(uhC / Tr, 2);

    ke = KR * alphaL * kInit;
    k0 = (1 - KR * alphaL) * kInit;
    double qYield = (1 - KR * alphaL) * fy;
    ...
    if (sqrt(pow(delta_ub(1), 2) + pow(delta_ub(2), 2)) > DBL_EPSILON) {
        ...
        do {
            ...
            if ((fabs(Df1(0, 0)) <= DBL_EPSILON) || (fabs(Df1(1, 1)) <= DBL_EPSILON)) {
                opserr << "WARNING: BoucWenHDRB::update() - "
        ...
        // set shear force
        qb(1) = ke * ub(1) + KH * qYield * z1(0) + KR * amp2 * z2(0);
        qb(2) = ke * ub(2) + KH * qYield * z1(1) + KR * amp2 * z2(1);
        // set tangent stiffness
        Kb(1, 1) = ke + KH * qYield * dz1du(0, 0) + KR * amp2 * dz2du(0, 0);
        Kb(1, 2) = KH * qYield * dz1du(0, 1) + KR * amp2 * dz2du(0, 1);
        Kb(2, 1) = KH * qYield * dz1du(1, 0) + KR * amp2 * dz2du(1, 0);
        Kb(2, 2) = ke + KH * qYield * dz1du(1, 1) + KR * amp2 * dz2du(1, 1);
    }
...
const Matrix& BoucWenHDRB::getTangentStiff()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& BoucWenHDRB::getInitialStiff()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& BoucWenHDRB::getDamp()
{
    // zero the matrix
    theMatrix.Zero();
...
const Matrix& BoucWenHDRB::getMass()
{
    // zero the matrix
    theMatrix.Zero();
...
void BoucWenHDRB::zeroLoad()
{
    theLoad.Zero();
}


int BoucWenHDRB::addLoad(ElementalLoad* theLoad, double loadFactor)
{
    opserr << "BoucWenHDRB::addLoad() - "
...
int BoucWenHDRB::addInertiaLoadToUnbalance(const Vector& accel)
{
    // check for quick return
    if (mass == 0.0) {
        return 0;
    ...
    if (6 != Raccel1.Size() || 6 != Raccel2.Size()) {
        opserr << "BoucWenHDRB::addInertiaLoadToUnbalance() - "
...
const Vector& BoucWenHDRB::getResistingForce()
{
    // zero the residual
    theVector.Zero();
...
const Vector& BoucWenHDRB::getResistingForceIncInertia()
{
...
int BoucWenHDRB::sendSelf(int commitTag, Channel& sChannel)
{
...
int BoucWenHDRB::recvSelf(int commitTag, Channel& rChannel,
...
int BoucWenHDRB::displaySelf(Renderer& theViewer,
...
void BoucWenHDRB::Print(OPS_Stream& s, int flag)
{
    if (flag == OPS_PRINT_CURRENTSTATE) {
        ...
        s << "  DSplus: " << DSplus << "  DSminus: " << DSminus << endln;
        s << "  DS: " << DS << "  ubMax: " << ubMax << endln;
        s << "  KR: " << KR << "  KH: " << KH << endln;
        s << "  amp2: " << amp2 << endln;
        // determine resisting forces in global system
...
Response* BoucWenHDRB::setResponse(const char** argv, int argc,
    OPS_Stream& output)
{
    Response* theResponse = 0;

    output.tag("ElementOutput");
    output.attr("eleType", "BoucWenHDRB");
        ...
        strcmp(argv[0], "parameters") == 0 || strcmp(argv[0], "Parameters") == 0)
    {
        ...
        output.tag("ResponseType", "ubMax");
        output.tag("ResponseType", "KR");
        output.tag("ResponseType", "KH");
        output.tag("ResponseType", "amp2");

        theResponse = new ElementResponse(this, 9, Vector(7));
    }
...
int BoucWenHDRB::getResponse(int responseID, Information& eleInfo)
{
    double kGeo1, MpDelta1, MpDelta2, MpDelta3, MpDelta4, MpDelta5, MpDelta6;
    Vector z(4), dzduVec(8), kbVec(4), params(7);
    ...
    case 9:  // parameters that vary with time
        ...
        params(2) = DS / Tr; params(3) = ubMax;
        params(4) = KR; params(5) = KH;
        params(6) = amp2;
...
double BoucWenHDRB::sgn(double x)
{
    if (x > 0)
        return 1.0;
    else if (x < 0)
